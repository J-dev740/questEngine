...
export class OnChainService {
	...
	async _verifyNFTcount(walletId: string, minCount: number): Promise<boolean> {
		const knn3Result: any =
		 await this.graphqlQueryService.getNFTdata(this.knn3Provider, walletId);
		console.log("knn3Result", knn3Result.data);
		if (
			knn3Result.data &&
	...
	async verifyENScount(walletId: string, minCount: number) {
		...
		console.log("kkn3Result",knn3Result);
