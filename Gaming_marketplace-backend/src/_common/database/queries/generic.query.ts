...
export class GenericQueryService<T extends Document> {
	...
	async readEntity(identifier: object): Promise<T> {
		if (await this.checkValidity(identifier)) {
			// console.log(
			// 	"readEntity_GenericQueeryService_questData",
			// 	this.model.findOne(identifier),
			// );
	...
	async updateEntity(identifier: object, data: object, arrayFilter?: object[]): Promise<object> {
		if (await this.checkValidity(identifier)) {
			return this.model.findOneAndUpdate(identifier, data, {
				new: true,
	...
	async checkValidity(identifier: object): Promise<boolean> {
		...
		console.log("exists_checkValidity", exists);
