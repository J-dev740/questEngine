...
import { PurchasedCoursesCard } from "../courseCard.purchased";
import { walletAddressSelector } from "../../../features/auth/auth.selectors";

const ProfilePurchasedCoursesList = () => {
	...
	return (
		<CardsList className="px-4">
			{data?.map((currElement, index) => {
				return <PurchasedCoursesCard data={currElement} />;
			})}
		</CardsList>
	);
};

export default ProfilePurchasedCoursesList;
