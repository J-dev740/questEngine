...
import { TimeCalender } from "../../../../common/form/calender/timeCalender";
import { Dropzone } from "../../../../common/form/dropzone";
import { InputField } from "../../../../common/form/inputField";
import { apolloClient } from "../../../../util/init/apollo.config";
import { limitedEssenceSchema } from "../essence.validation";
...
const LimitedEssenceForm = ({ onResult, primaryProfile }: Props) => {
	const [icon, setIcon] = useState<File | null>(null);
	const [errors, setErrors] = useState<{ [k: string]: string | null }>({});
	const [calStart, setCalStart] = useState<Date>(new Date());
	const [calEnd, setCalEnd] = useState<Date>(new Date());
	...
	const handleSubmit = async (e: React.FormEvent<RegisterForm>) => {
		...
		try {
			const result = await limitedEssenceSchema.validate(
				{
					text: text.value,
					image: icon,
					price: price.value,
					...
					input: {
						...
						middleware: {
							collectLimitedTimePaid: {
								...
								price: ethers.utils.parseEther(result.price.toString()).toString(),
								currency: process.env.NEXT_PUBLIC_LINK_TOKEN_ADDR as string, // LINK token
								startTimestamp: Math.round(calStart.getTime() / 1000),
								endTimestamp: Math.round(calEnd.getTime() / 1000),
...
export default LimitedEssenceForm;
