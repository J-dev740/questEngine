...
import { GET_COLLECTED_ESSENCES } from "../../../../features/cyber-connect/graphql/GetCollectedEssences";
...
const UserCollectedNFTList = ({ primaryProfile }: Props) => {
	const walletAddress = useSelector(walletAddressSelector);
	const { data, loading, error } = useQuery(GET_COLLECTED_ESSENCES, {
		variables: {
	...
	const essences: IEssenceCard[] = data?.address?.wallet?.collectedEssences?.edges?.map(
		(edge: any) => edge?.node,
	);
...
export default UserCollectedNFTList;
